...
* Seamless Zooming: Instantly scale from micro-planning (5-minute intervals, similar to a video editing timeline) to a macro-overview (the entire year on one screen).
* Infinite 2D Canvas: Move freely along the X and Y axes. Days, weeks, and months flow into one another with zero breaks or cuts between years.
* Smart Macro View: In the yearly scale, each month flows into the next as a continuous row, while columns align by days of the week for maximum clarity.
* 2-Click Repetition: A highly intuitive UX designed for setting up complex recurring events and shift schedules in just two clicks.
